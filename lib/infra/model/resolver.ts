...
import { getUserModelById } from '@/lib/db/user-models';
...
/**
 * 解析用户模型
 * 根据模型 ID 和用户 ID 从数据库获取用户模型配置并创建 LanguageModel
 * @param modelId - 模型 ID
 * @param userId - 用户 ID
 * @returns 解析结果
 */
export async function resolveUserModel(
  ...
  try {
    // 1. 从数据库获取用户模型
    const userModel = await getUserModelById(userId, modelId);
    ...
    // 3. 创建模型实例
    const model = provider(userModel.model);
    ...
    console.error('用户模型解析失败:', error);
    return {
      ok: false,
      error: error instanceof Error ? error.message : '用户模型解析失败',
    };
  }
}
