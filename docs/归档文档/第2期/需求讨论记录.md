...
1. **重复配置**：`config.ts` 和 `tools.ts` 都定义了 Agent 工具配置
2. **未使用代码**：`chat-area.tsx`、`createMessages()` 等定义但未使用
3. **MCP资源清理时机错误**：`finally` 块提前关闭客户端

### 解决方案

#### 1. 删除重复配置
统一在 `config.ts` 管理 Agent 配置（包括 tools），删除 `tools.ts` 中的重复定义。

#### 2. 删除未使用代码
| 文件 | 删除内容 |
|------|----------|
| `components/chat/chat-area.tsx` | 整个文件 |
| `lib/db/messages.ts` | createMessages, getLastMessage, getMessageCount |
| `lib/agents/config.ts` | isValidAgentId |

#### 3. MCP资源清理时机
- 移除 `finally` 块
- 在 `onFinish` 回调中清理（响应完成后）
- 在 `catch` 块中清理（错误时）
...
**原始流程**：
1. 用户访问 `/chat` → `ChatClient({ id: null })`
2. 发送首条消息 → API 创建新对话
3. `window.history.replaceState()` 更新 URL
4. **问题**：组件的 `id` prop 仍为 `null`
5. 发送第二条消息 → API 又创建新对话

**根本原因**：
`replaceState()` 只更新浏览器地址栏 URL，不会触发组件重新渲染，props 不会更新。

### 解决方案

参考官方 [ai-chatbot](https://github.com/vercel/ai-chatbot) 项目的实现：
...
| 场景 | 操作 | 说明 |
|------|------|------|
| 新对话页面 | 预生成 ID | 避免首条消息后重复创建 |
| 发送首条消息 | pushState | 无刷新更新 URL |
| 发送后续消息 | 使用同一 ID | 正确关联到同一对话 |
| 浏览器前进/后退 | popstate + refresh | 同步 URL 和状态 |
...
## 讨论14：AgentSelector 禁用逻辑修正

**日期**：2026-02-23

### 发现的问题
修改后 `id` 总是有值，导致 `disabled={!!id}` 始终为 `true`，新建对话时也无法选择 Agent。

### 解决方案
使用 `currentConversation` 判断是否为已有对话：
```typescript
<AgentSelector
...
**逻辑说明**：
- `currentConversation` 来自数据库查询结果
- 新对话页面（`/chat`）：不从数据库加载，`currentConversation` 为 `undefined`，Agent 可选
- 已有对话页面（`/chat/[id]`）：从数据库加载，`currentConversation` 有值，Agent 禁用

### 验证结果
✅ 新对话可以选择 Agent
✅ 已有对话 Agent 选择器禁用但显示当前 Agent
